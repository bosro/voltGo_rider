/**
 * PackageCollectedScreen.tsx — Real API integration
 * "Package collected" → POST /rider/orders/{id}/collected
 *                    → POST /rider/orders/{id}/in-transit (on navigate)
 */
...
import { useMarkCollected, useMarkInTransit } from '../../hooks/rider/useOrders';
...
export default function PackageCollectedScreen() {
  ...
  const {
    ...
    pickupAddress, dropoffAddress, itemType, price,
  ...
  const handleCollected = async () => {
    try {
      ...
      // Fire in-transit in background — no need to await
  ...
  const displayEta = etaMinutes ?? 15;
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.View style={[styles.card, { opacity: fadeIn, transform: [{ translateY: slideUp }] }]}>
        ...
        <TouchableOpacity style={styles.actionBtn} onPress={handleCollected} activeOpacity={0.88} disabled={isCollecting}>
          {isCollecting
...
const styles = StyleSheet.create({
  ...
  pickupDot: { width: 13, height: 13, borderRadius: 6.5, backgroundColor: '#4A90E2', borderWidth: 2.5, borderColor: Colors.white },
