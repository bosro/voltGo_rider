...
import {
  View, Text, StyleSheet, TouchableOpacity, StatusBar, Alert, Image,
} from 'react-native';
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.profileCard}>
        <View style={styles.avatarPlaceholder}>
          {rider?.avatar_url
            ? <Image source={{ uri: rider.avatar_url }} style={styles.avatarImage} />
            : <Text style={{ fontSize: 24 }}>👤</Text>
          }
        </View>
        <View style={styles.profileText}>
          <Text style={styles.profileName}>{rider?.name ?? 'Rider'}</Text>
          <Text style={styles.profileEmail}>{rider?.email ?? rider?.phone ?? ''}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  heading: {
    fontFamily: 'HelveticaNeue-CondensedBold', fontSize: Typography.xl,
    color: Colors.textPrimary, textAlign: 'center', paddingTop: 16, paddingBottom: 16,
  },
  profileCard: {
    flexDirection: 'row', alignItems: 'center',
    marginHorizontal: 22, backgroundColor: Colors.white,
    borderRadius: Radius.xl, paddingVertical: 14, paddingHorizontal: 16,
    gap: 14, marginBottom: 18, ...Shadow.card, borderWidth: 1, borderColor: Colors.border,
  },
  avatarPlaceholder: {
    width: 58, height: 58, borderRadius: 29, backgroundColor: Colors.inputBg,
    alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
  },
  avatarImage: { width: 58, height: 58, borderRadius: 29 },
  profileText: { flex: 1 },
  profileName: { fontFamily: 'Poppins-Bold', fontSize: Typography.lg, color: Colors.textPrimary, marginBottom: 2 },
  profileEmail: { fontFamily: 'Poppins-Regular', fontSize: Typography.sm, color: Colors.textMuted },
  menuList: { paddingHorizontal: 22 },
  menuRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 17, gap: 14 },
  menuLabel: { flex: 1, fontFamily: 'Poppins-Regular', fontSize: Typography.base, color: Colors.textPrimary },
