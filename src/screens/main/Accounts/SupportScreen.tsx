...
const FAQS = [
  { q: 'How do I receive payments?', a: 'Payments are credited to your wallet after each completed delivery. You can withdraw anytime from the Wallet screen to your registered mobile money or bank account.' },
  { q: 'What happens if I miss an order?', a: 'Missed orders are returned to the pool. Repeated missed orders may affect your acceptance rate, so only go online when you are ready to take deliveries.' },
  { q: 'How long does withdrawal take?', a: 'Mobile Money withdrawals are instant. Bank transfers take 1–2 business days.' },
  { q: 'What do I do if a customer is unreachable?', a: 'Try calling and messaging at least twice. If still unreachable, take a photo at the drop-off location and tap "Package collected" to document the attempt.' },
  { q: 'How is my earnings calculated?', a: 'Earnings are based on distance, vehicle type, and package weight. You can see the breakdown in each completed delivery in Activities.' },
  { q: 'How do I report an issue with an order?', a: 'Tap the chat icon during an active delivery to contact our dispatch team, or reach us via WhatsApp or email below.' },
];
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        {[
          { icon: whatsappSvg, label: 'WhatsApp', sub: '+233 55 123 4567', onPress: () => Linking.openURL('https://wa.me/233551234567') },
          { icon: emailSvg2, label: 'Email', sub: 'support@voltgo.com', onPress: () => Linking.openURL('mailto:support@voltgo.com') },
          { icon: phoneSvg2, label: 'Call us', sub: '+233 55 123 4567', onPress: () => Linking.openURL('tel:+233551234567') },
        ].map((item, i) => (
          <TouchableOpacity key={i} style={styles.contactCard} onPress={item.onPress} activeOpacity={0.8}>
            <View style={styles.contactIcon}><SvgXml xml={item.icon} width={22} height={22} /></View>
        ...
        <Text style={[styles.sectionLabel, { marginTop: 24 }]}>FREQUENTLY ASKED QUESTIONS</Text>
        {FAQS.map((faq, i) => (
          <View key={i} style={styles.faqItem}>
            <TouchableOpacity style={styles.faqQuestion} onPress={() => setOpenFaq(openFaq === i ? null : i)} activeOpacity={0.8}>
              <Text style={styles.faqQ}>{faq.q}</Text>
              <SvgXml xml={openFaq === i ? chevronUpSvg : chevronDownSvg} width={14} height={9} />
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  contactCard: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: Colors.border, borderRadius: Radius.lg, padding: 16, marginBottom: 12, gap: 14, ...Shadow.card },
  contactIcon: { width: 44, height: 44, borderRadius: 22, backgroundColor: Colors.inputBg, alignItems: 'center', justifyContent: 'center' },
  contactText: { flex: 1 },
  contactLabel: { fontFamily: 'Poppins-SemiBold', fontSize: Typography.base, color: Colors.textPrimary },
  contactSub: { fontFamily: 'Poppins-Regular', fontSize: Typography.sm, color: Colors.textMuted },
  faqItem: { paddingVertical: 4 },
  faqQuestion: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 14 },
  faqQ: { fontFamily: 'Poppins-SemiBold', fontSize: Typography.base, color: Colors.textPrimary, flex: 1, marginRight: 12 },
  faqA: { fontFamily: 'Poppins-Regular', fontSize: Typography.base, color: Colors.textSecondary, lineHeight: 22, paddingBottom: 14 },
